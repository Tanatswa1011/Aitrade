"""Download Databento CL.v.0 ohlcv-1m 2020-2026 for Phase 46 (quote first)."""
...
OUT = ROOT / "data" / "databento" / "CL" / "stitched"
START = "2020-01-01"
END = "2026-08-17"
...
def main() -> dict:
    ...
    existing = OUT / "databento_CL_v0_1m.jsonl"
    ...
    dbn = OUT / "databento_CL_v0_1m.dbn.zst"
    ...
    written = write_dataset(
        bars,
        symbol="databento_CL_v0",
        timeframe="1m",
        source="databento:GLBX.MDP3:CL.v.0_volume_continuous",
        root=OUT,
        expected_period_sec=60,
    )
    meta_path = OUT / "databento_CL_v0_1m.meta.json"
    meta = json.loads(meta_path.read_text(encoding="utf-8"))
    meta.update({
        "root": "CL",
        "schema": "ohlcv-1m",
        "continuous_choice": "databento_CL.v.0_volume_continuous",
        "tick_size": 0.01,
        "phase": 46,
        "period": f"{START} -> {END}",
        "cost_usd": float(cost),
