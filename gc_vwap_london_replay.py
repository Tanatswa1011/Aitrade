"""Replay Phase 27 London VWAP candidates into journal rows (isolated journal)."""
...
def setup_to_journal(setup, bars: Sequence[Bar], cfg: GCVWAPStrategyConfig) -> SetupJournalRecord:
    ...
    _, session_end, _ = london_session_window(setup.trading_date)
    ...
    if setup.entry_triggered and setup.entry_timestamp and setup.stop_price is not None:
        vwap_touch = evaluate_london_vwap_touch_after_entry(
            bars=bars,
    ...
    return SetupJournalRecord(
        ...
        trading_date=setup.trading_date,
        session="GC_VWAP_LONDON_0800_1200",
        direction=setup.direction,
        ...
        confirmation_source="gc_vwap_london_mean_reversion",
        ...
        reliability_flags=["STRATEGY_FAMILY_GC_VWAP_LONDON_MEAN_REVERSION_V1"]
        ...
        extras={
            ...
            "session_timezone": LONDON_SESSION.timezone,
            "session_window": f"{LONDON_SESSION.start_local}-{LONDON_SESSION.end_local}",
            "not_phase26_ny_v2": True,
        },
    )


def replay_all_london_candidates(
    bars: Sequence[Bar],
    candidates=PHASE27_CANDIDATES,
    ...
    for cfg in candidates:
        recs = []
        for seq in seqs:
            setup = analyze_london_candidate(seq, cfg)
